...
app.post('/salesmen', (req, res) => {
  ...
  const existingSalesman = salesmen.find(salesman => salesman.sid === newSalesman.sid);
  if (existingSalesman) {
    return res.status(400).json({ message: 'Salesman with this SID already exists' });
  }
  ...
  for (let goal of newSalesman.SPR) {
    ...
    if (goalExists) {
      return res.status(400).json({ message: `GoalId ${goal.goalId} already exists for another salesman` });
    }
  }
